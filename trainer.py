...
class MUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters, device):
        ...
        self.s_a = torch.randn(display_size, self.style_dim, 1, 1).cuda(self.device)
        self.s_b = torch.randn(display_size, self.style_dim, 1, 1).cuda(self.device)
    ...
    def sample(self, x_a, x_b):
        ...
        s_a2 = Variable(torch.randn(x_a.size(0), self.style_dim, 1, 1).cuda(self.device))
        s_b2 = Variable(torch.randn(x_b.size(0), self.style_dim, 1, 1).cuda(self.device))
