...
def get_logits(input_folder, input_list, transform, dis):
    ...
    for i in input_list:
        outputs.append([])
        img = Image.open(os.path.join(input_folder, i))
        img = transform(img)
        img = img.to(device)
...
input_real = os.listdir(opts.real_input_folder)
input_fake = os.listdir(opts.fake_input_folder)

output_real = get_logits(opts.real_input_folder, input_real, transform, dis)
output_fake = get_logits(opts.fake_input_folder, input_fake, transform, dis)

save_logits(opts.output_folder, output_real, 'Logits of real images')
save_logits(opts.output_folder, output_fake, 'Logits of fake images')
